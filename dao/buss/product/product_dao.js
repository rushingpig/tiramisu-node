...
ProductDao.prototype.findProductsCount = function(product_name,category_id,regionalism_id, isAddition){
    let sql = "",params = [];
    sql += "select bp.id,bps.size,bp.name,bpc.name as category_name,bpc.isAddition,bps.regionalism_id ";
    sql += " from ?? bp";
...
ProductDao.prototype.getSkuWithBooktimeByProductAndCity = function (data) {
    ...
    let sql = 'select ' + columns.join(',') + ' from ?? sku left join ?? secondary_booktime on sku.id = secondary_booktime.sku_id left join ?? dict on secondary_booktime.regionalism_id = dict.id where sku.product_id = ? and sku.regionalism_id = ? and sku.del_flag = 1 and secondary_booktime.del_flag = 1 ';
    let params = [];
...
ProductDao.prototype.getSkuByProductWithRegion = function (productId) {
    ...
    let sql = 'select ' + columns.join(',') + ' from ?? sku left join ?? secondary_booktime on sku.id = secondary_booktime.sku_id where sku.product_id = ? and sku.del_flag = 1 and secondary_booktime.del_flag = 1';
    let params = [];
    params.push(config.tables.buss_product_sku);
    params.push(config.tables.buss_product_sku_booktime);
    params.push(productId);
