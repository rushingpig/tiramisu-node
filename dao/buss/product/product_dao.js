...
ProductDao.prototype.getSkuWithBooktimeByProductAndCity = function (data) {
    ...
    let sql = 'select ' + columns.join(',') + ' from ?? sku left join ?? secondary_booktime on sku.id = secondary_booktime.sku_id and secondary_booktime.del_flag = 1 left join ?? dict on secondary_booktime.regionalism_id = dict.id where sku.product_id = ? and sku.regionalism_id = ? and sku.del_flag = 1';
    let params = [];
