...
function AddressDao(table){
    this.table = table;
    this.baseSql = util.format('select ?? from %s where 1=1 and level_type = ? ',this.table);
...
AddressDao.prototype.findAllProvinces = function(){
    return baseDao.select(this.baseSql,[this.baseColumns,1]);
};
AddressDao.prototype.findCitiesByProvinceId = function(provinceId){
    let sql = this.baseSql + ' and parent_id = ?';
    return baseDao.select(sql,[this.baseColumns,2,provinceId]);
};
AddressDao.prototype.findDistrictsByCityId = function(cityId){
    let sql = this.baseSql + ' and parent_id = ?';
    return baseDao.select(sql,[this.baseColumns,3,cityId]);
};
