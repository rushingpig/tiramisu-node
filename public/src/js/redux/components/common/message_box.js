...
const MessageBox = React.createClass({
  ...
  render() {
    ...
    switch (thisProps.icon) {
      case MessageBoxIcon.Success:
        iconArea = <span className='fa fa-check-circle text-success' />
        break;
      case MessageBoxIcon.Info:
        iconArea = <span className='fa fa-info-circle text-info' />
        break;
      case MessageBoxIcon.Error:
        iconArea = <span className='fa fa-times-circle text-danger' />
        break;
      case MessageBoxIcon.Warning:
        iconArea = <span className='fa fa-warning text-warning' />
        break;
      case MessageBoxIcon.Question:
        iconArea = <span className='fa fa-question-circle text-primary' />
        break;
  ...
  handleClickBtn(action) {
    action ? this.props.resolve() : this.props.reject();
  }
})
...
var showMessageBox = function({ icon, text, title, btnType } = {}) {
  ...
  promise = new Promise(function(resolve, reject) {
    ...
    ReactDOM.render(
      <MessageBox icon={icon} text={text} title={title} type={btnType} resolve={resolve} reject={reject} />,
      wrap
    )
  });
